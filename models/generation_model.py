...
from utils import pose_utils,warp_image
...
class GenerationModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # self.resume_path = [resume_gmm] ## TODO  Trinanjan change this 
        for index, (network, resume) in enumerate(zip(self.networks_model, self.resume_path)):
            if osp.exists(resume):
                print("path exists so loading model {} from {}".format(self.networks_name[index],resume))
                #assert(osp.exists(resume), 'the resume not exits')
                self.load_network(network, resume, ifprint=False)
                print('Model Loaded ...................................')
                
            else :
                print("{} model path not found. Random initialized the mentioned normalization".format(self.networks_name[index]))
    ...
    def set_input(self, opt, result):
        ...
        # Input variable returned by dataloader
        ...
        self.source_densepose_data = result['source_densepose_data'].float().cuda()
        self.target_densepose_data = result['target_densepose_data'].float().cuda()
        ...
        self.input_parsing = torch.cat((real_s_, self.target_densepose_data, self.cloth_parse), 1).cuda()
        
        # For GMM training we need agnostic cloth_represent(source_head, keypoints) original_cloth (from dataloader) 
        if opt.train_mode == 'gmm':
            self.im_h = result['im_h'].float().cuda()
            self.source_parse_shape = result['source_parse_shape'].float().cuda()
            self.agnostic = torch.cat((self.source_parse_shape, self.im_h, self.target_densepose_data), dim=1)
        ...
        if opt.train_mode != 'parsing' and opt.train_mode != 'gmm':
            self.warped_cloth = warped_image(gmm, result)
            ...
            # print('setting input inside appearance model')
        ...
        # NOT USED 
        # For joint training we are calling train_mode appearance --joint_all --joint_parse_loss
    ...
    # Train the generator/generators(if joint_all is true)

    def backward_G(self, opt):
        self.t6 = time()

        # Loss Function for gmm    
        if opt.train_mode == 'gmm':
        ...
        ############### Loss function for parsing generator network
    ...
    # Train the Discriminator(there is always one discriminator to train)
    ...
    def save_result(self, opt, epoch, iteration):        
        if opt.train_mode == 'gmm':
            images = [self.cloth_image, self.im_c, self.warped_cloth_predict.detach()]

        if opt.train_mode == 'parsing':
            fake_t_vis = pose_utils.decode_labels(torch.argmax(self.fake_t, dim=1, keepdim=True).permute(0,2,3,1).contiguous())
            images = [self.source_parse_vis, self.target_parse_vis, self.target_densepose_data, self.cloth_parse, fake_t_vis]
