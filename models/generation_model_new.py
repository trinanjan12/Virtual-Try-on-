...
class GenerationModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if opt.train_mode == 'gmm':
            ...
            self.optimizer_G = [self.optimizer_parsing_G,
    ...
    def set_input(self, opt, result):
        ...
        real_s_ = torch.index_select(
            self.source_parse, 1, torch.tensor(index).cuda())
        ...
        if opt.train_mode == 'gmm':
            ...
            self.agnostic = torch.cat(
                (self.source_parse_shape, self.im_h, self.target_pose_embedding), dim=1)
   ...
   # All back propagation and loss operations Discriminator networks
    ...
    def save_result(self, opt, epoch, iteration):
        ...
        ######################################
        if opt.train_mode == 'gmm':
            images = [self.target_pose_img, self.cloth_image, self.im_c,
        ...
        if opt.train_mode == 'parsing':
            ...
            images = [self.source_parse_vis, self.target_parse_vis,
        ...
        pose_utils.save_img(images, os.path.join(
            self.vis_path, str(epoch) + '_' + str(iteration) + '.jpg'))
    ...
    def print_current_errors(self, opt, epoch, i, iteration):
        ...
        if opt.train_mode == 'appearance':
            ...
            if opt.joint_all and opt.joint_parse_loss:
                ...
                for key in errors:
                    self.writer.add_scalar('Loss/JOINTALL/' + str(key),errors[key], iteration)
            else:
                errors = {'loss_G': self.loss_G.item(), 'loss_G_GAN': self.loss_G_GAN.item(), 'loss_G_vgg': self.loss_G_vgg.item(), 'loss_G_mask': self.loss_G_mask.item(),
                ...
                for key in errors:
                    self.writer.add_scalar('Loss/APPEARANCE/' + str(key),errors[key], iteration)
